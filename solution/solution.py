...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "Score the answer according to the rubric. Return JSON mapping "
            "each criterion to a numeric score from 0 to 1.\n\n"
            f"Question:\n{question}\n\n"
            f"Answer:\n{answer}\n\n"
            f"Rubric:\n{rubric_lines}"
        )
        ...
        try:
            ...
            if not isinstance(raw_scores, dict):
                raise ValueError("judge response did not contain scores")
            scores = {
            ...
            if not scores:
                raise ValueError("judge response had no numeric scores")
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            ...
            for key, value in scores.items():
                ...
                if key_lower.startswith(("first", "response_1", "answer_1", "a_")):
                    first_scores.append(score)
                elif key_lower.startswith(("second", "response_2", "answer_2", "b_")):
        ...
        if first_scores and other_scores:
            positional_bias = (
                ...
                > sum(other_scores) / len(other_scores) + 0.05
            )
        return {
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_context_precision": avg(precision_values)
            if precision_values
            else None,
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        baseline_avgs = {
            metric: avg_metric(baseline_results, metric) for metric in metrics
        }
        regressions = [
            metric
...
class FailureAnalyzer:
    ...
    def categorize_failures(
        ...
        """
        return dict(
            Counter(failure.failure_type or "unknown" for failure in failures)
        )
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        if len(sorted_scores) >= 2 and sorted_scores[1][1] - sorted_scores[0][1] <= 0.05:
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination", 0):
            suggestions.append(
                "Add a grounding check that removes claims not supported by retrieved contexts"
            )
        if categories.get("irrelevant", 0) or categories.get("off_topic", 0):
            suggestions.append(
                "Tighten the answer prompt to restate the user intent and answer only supported OrbitTech policy questions"
            )
        if categories.get("incomplete", 0):
            suggestions.append(
                "Increase top-k or chunk coverage for multi-policy questions that need several conditions and exceptions"
            )

        if len(suggestions) < 3:
            suggestions.append(
                "Add representative failed cases to the golden dataset and run regression checks before deployment"
            )
        if len(suggestions) < 3:
            suggestions.append(
                "Review retrieved chunk ranking and introduce lexical or cross-encoder reranking for noisy traces"
            )
        if len(suggestions) < 3:
            suggestions.append(
                "Calibrate thresholds with human labels so quality gates match support risk"
            )
        return suggestions
